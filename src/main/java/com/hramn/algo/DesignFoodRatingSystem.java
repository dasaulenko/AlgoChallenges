 ...
 *   that has the highest rating for the given type of cuisine. If there is 
 *   a tie, return the item with the lexicographically smaller name.
 * 
 ...
 * if i is the first position such that x[i] != y[i], then x[i] comes before 
...
public class DesignFoodRatingSystem {
	class FoodRatings {
		...
		private Comparator<Food> comporator = new Comparator<>() {
			public int compare(Food obj1, Food obj2) {
				int dif = obj2.rating - obj1.rating;
				if (dif != 0) {
					return dif;
				}
				return obj1.food.compareTo(obj2.food);
			}
		};
		private Map<String, PriorityQueue<Food>> cuisineToRating = new HashMap<>();
		private Map<String, Food> foodMap = new HashMap<>();

		public FoodRatings(String[] foods, String[] cuisines, int[] ratings) { // O(N * log(N)), O(N) where N = foods.length
			for (int i = 0; i < foods.length; i++) {
				Food f = new Food(foods[i], cuisines[i], ratings[i]);
				PriorityQueue<Food> queue = cuisineToRating.getOrDefault(
					f.cuisine, 
					new PriorityQueue<>(comporator));
				queue.add(f);
				foodMap.put(f.food, f);
				cuisineToRating.put(f.cuisine, queue);
			}
		}
    
		public void changeRating(String food, int newRating) { // O(log(N)), O(1) where N = queue.length
			Food f = foodMap.get(food);
			Food newF = new Food(food, f.cuisine, newRating);
			foodMap.put(food, newF);
			PriorityQueue<Food> queue = cuisineToRating.get(f.cuisine);
			queue.add(newF);
		}

		public String highestRated(String cuisine) { // O(N), O(1) where N = queue.length
			PriorityQueue<Food> queue = cuisineToRating.get(cuisine);
			Food f = queue.peek();
			while (foodMap.get(f.food) != f) {
				queue.poll();
				f = queue.peek();
			}
			return f.food;
