...
public class AdjacentIncreasingSubarraysDetectionI {
	class Solution { // O(N), O(1), where N = nums.size()
		public boolean hasIncreasingSubarrays(List<Integer> nums, int k) {
			...
			int prevIncSubarrayLength = 0;
			int curentIncSubarrayLength = 1;
			int prevNum = iterator.next();
			while (iterator.hasNext()) {
				int num = iterator.next();
				if (num <= prevNum) {
					prevIncSubarrayLength = curentIncSubarrayLength;
					curentIncSubarrayLength = 1;
				} else {
					curentIncSubarrayLength++;
				}
				if ((prevIncSubarrayLength >= k && curentIncSubarrayLength >= k) 
					|| (curentIncSubarrayLength / 2 >= k)) return true;
				prevNum = num;
			}
