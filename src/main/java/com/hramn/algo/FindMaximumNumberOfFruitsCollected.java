 ...
 * the number of fruits in the room (i, j). Three children will play in 
 * the game dungeon, with initial positions at the corner rooms (0, 0), 
 * (0, n - 1), and (n - 1, 0).
 * 
 ...
 * or more children enter the same room, only one child will collect 
...
public class FindMaximumNumberOfFruitsCollected {
	class Solution { // O(N^2), O(N^2)
		public int maxCollectedFruits(int[][] fruits) {
			...
			// b2
			reqMaxB(new int[] {n-1,n-1}, fruits, dp);
			b2 = dp[n-1][n-1];
			...
			// b3
			reqMaxC(new int[] {n-1,n-1}, fruits, dp);
			b3 = dp[n-1][n-1];

			return b1 + b2 + b3;
    }

		void reqMaxB(int[] p, int[][] fruits, int[][] dp) {
			int n = fruits.length;
			dp[p[0]][p[1]] = 0;
			if (p[0] - 1 >= 0) {
				if (p[1]+1 < n) {
					if (dp[p[0]-1][p[1]+1] < 0) {
						reqMaxB(new int[] {p[0]-1,p[1]+1}, fruits, dp);
					}
					dp[p[0]][p[1]] = Math.max(dp[p[0]][p[1]], dp[p[0]-1][p[1]+1]);
				}

				if (p[1] >= n - p[0]) {
					if (dp[p[0]-1][p[1]] < 0) {
						reqMaxB(new int[] {p[0]-1,p[1]}, fruits, dp);
					}
					dp[p[0]][p[1]] = Math.max(dp[p[0]][p[1]], dp[p[0]-1][p[1]]);
				}
			...
			dp[p[0]][p[1]] += fruits[p[0]][p[1]];
		}

		void reqMaxC(int[] p, int[][] fruits, int[][] dp) {
			int n = fruits.length;
			dp[p[0]][p[1]] = 0;
			if (p[1] - 1 >= 0) {
				if (p[0]+1 < n) {
					if (dp[p[0]+1][p[1]-1] < 0) {
						reqMaxC(new int[] {p[0]+1,p[1]-1}, fruits, dp);
					}
					dp[p[0]][p[1]] = Math.max(dp[p[0]][p[1]], dp[p[0]+1][p[1]-1]);
				}
				...
				if (p[0] >= n - p[1]) {
					if (dp[p[0]][p[1]-1] < 0) {
						reqMaxC(new int[] {p[0],p[1]-1}, fruits, dp);
					}
					dp[p[0]][p[1]] = Math.max(dp[p[0]][p[1]], dp[p[0]][p[1]-1]);
				}
