 ...
 * friends can communicate with each other. Return the minimum number of users 
 * you need to teach.
 * 
...
public class MinimumNumberOfPeopleToTeach {
	class Solution { // O(K * N), O(M+N) where N = n, M = languages.length, K = friendships.length
		public int minimumTeachings(int n, int[][] languages, int[][] friendships) {
			...
			for (int[] friendship : friendships) {
				...
				int[] lang1 = languages[f1];
				int[] lang2 = languages[f2];
				if (!findCrossing(lang1, lang2, n)) {
					friends.add(f1);
					friends.add(f2);
				}
			}

			int[] allLangs = new int[n];
			for (int friend : friends) {
				int[] langs = languages[friend];
				for (int l : langs) {
					allLangs[l-1]++;
				}
			}
			...
			int max = 0;
			for (int i = 0; i < n; i++) {
				max = Math.max(max, allLangs[i]);
			}

			return friends.size() - max;
		}

		private boolean findCrossing(int[] lang1, int[] lang2, int n) {
			boolean[] lang = new boolean[n];
			for (int l : lang1) {
				lang[l-1] = true;
			}
			for (int l : lang2) {
				if (lang[l-1]) return true;
			}
			return false;
		}
	}
