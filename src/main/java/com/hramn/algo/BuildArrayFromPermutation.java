...
public class BuildArrayFromPermutation {
	...
	class Solution { // O(N), O(1), N - nums.length
		// nums[i] = first number + (second number * factor), where factor >= max 
		// first number
		// to get first number: nums[i] % factor
		// to get second number: nums[i] / factor
	...
	class BruteforceSolution { // O(N), O(N), N - nums.length
		public int[] buildArray(int[] nums) {
