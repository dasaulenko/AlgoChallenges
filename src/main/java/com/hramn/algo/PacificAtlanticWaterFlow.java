...
public class PacificAtlanticWaterFlow {
	class Solution {
		public List<List<Integer>> pacificAtlantic(int[][] heights) {
			...
			for (int i = 0; i < m; i++) {
				dp[i][0] = 1;
				queue.add(new int[] {heights[i][0], i, 0});
			}
			for (int i = 1; i < n; i++) {
				dp[0][i] = 1;
				queue.add(new int[] {heights[0][i], 0, i});
			}
			...
			for (int i = 0; i < m; i++) {
				dp[i][n-1] += 2;
				queue.add(new int[] {heights[i][n-1], i, n-1});
			}
			for (int i = 0; i < n-1; i++) {
				dp[m-1][i] += 2;
				queue.add(new int[] {heights[m-1][i], m-1, i});
			}
			while (!queue.isEmpty()) {
				int[] u = queue.poll();
				int h = u[0];
				...
				for (int[] d : dir) {
					int ni = i + d[0];
					int nj = j + d[1];
					if (ni < 0 || ni >= m || nj < 0 || nj >= n || dp[ni][nj] > 1) 
						continue;
					int nh = heights[ni][nj];
					if (h <= nh) {
						dp[ni][nj] += 2;
