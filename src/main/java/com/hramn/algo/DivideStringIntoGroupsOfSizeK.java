 ...
 *   so on. Each element can be a part of exactly one group.
...
public class DivideStringIntoGroupsOfSizeK {
	class Solution { // O(N), O(1), N - s.length
		public String[] divideString(String s, int k, char fill) {
			int n = s.length();
			int len = n / k;
			String[] result = new String[len + (n % k == 0 ? 0 : 1)];
			for (int i = 0; i < len; i++) {
				result[i] = s.substring (i*k, i*k+k);
			}
			if (len != result.length) {
				StringBuilder sb = new StringBuilder();
				sb.append(s.substring(len * k));
				for (int i = 0; i < k - (n - len*k); i++) {
					sb.append(fill);
				}
				result[len] = sb.toString();
			}
			return result;
		}
	}
