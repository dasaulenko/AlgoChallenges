 ...
 * the upper left corner and Bob's position as the lower right corner of 
 * the fence (Note that the fence might not enclose any area, i.e. it can be 
 ...
 * You cannot place Alice at (2, 6) and Bob at (6, 2) because the person 
 * at (4, 4) will be inside the fence.
 * 
 * Example 3:
 ...
 * You cannot place Alice at (1, 3) and Bob at (3, 1) because 
 * the person at (1, 1) will be on the fence.
			...
			for (int l = 0; l < n-1; l++) {
				...
				for (int r = l+1; r < n; r++) {
					if (points[r][1] > points[l][1]) {
						continue;
					} else if (points[r][1] == points[l][1]) {
						result++;
						break;
					} else if (points[r][1] > min) {
						min = points[r][1];
						result++;
					}
				}
