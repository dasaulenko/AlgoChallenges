package com.hramn.algo.numbers;

import java.util.Arrays;

/**
 * https://leetcode.com/problems/closest-prime-numbers-in-range/description
 * 
 * 2523. Closest Prime Numbers in Range
 * 
 * Given two positive integers left and right, find the two integers num1 and 
 * num2 such that:
 * - left <= num1 < num2 <= right .
 * - Both num1 and num2 are prime numbers.
 * - num2 - num1 is the minimum amongst all other pairs satisfying the above 
 *   conditions.
 * 
 * Return the positive integer array ans = [num1, num2]. If there are multiple 
 * pairs satisfying these conditions, return the one with the smallest num1 
 * value. If no such numbers exist, return [-1, -1].
 * 
 * Example 1:
 * Input: left = 10, right = 19
 * Output: [11,13]
 * Explanation: The prime numbers between 10 and 19 are 11, 13, 17, and 19.
 * The closest gap between any pair is 2, which can be achieved by [11,13] 
 * or [17,19].
 * Since 11 is smaller than 17, we return the first pair.
 * 
 * Example 2:
 * Input: left = 4, right = 6
 * Output: [-1,-1]
 * Explanation: There exists only one prime number in the given range, so the 
 * conditions cannot be satisfied.
 * 
 * Constraints:
 * 1 <= left <= right <= 10^6
 */
public class ClosestPrimeNumbersInRange {
	class BruteforceSolution { // O(N * N^(1/N)), O(1)
		// left = 5, right = 15
		// i = 5
		// i = 6
		// i = 7 ans[0] = 7
		// i = 8
		// i = 9
		// i = 10
		// i = 11 ans[1] = 11
		// i = 12
		// i = 13 ans[0] = 11 ans[1] = 13
		public int[] closestPrimes(int left, int right) {
			int[] ans = new int[2];
			int lastPrimeNum = 0;
			for (int i = left; i <= right; i++) {
				if (isPrimeNumber(i)) {
					if (ans[0] == 0) {
						ans[0] = i;
					} else if (ans[1] == 0) {
						ans[1] = i;
					} else {
						if (ans[1] - ans[0] > i - lastPrimeNum) {
							ans[0] = lastPrimeNum;
							ans[1] = i;
						}
					}
					lastPrimeNum = i; 
				}
			}
			return ans[0] > 0 && ans[1] > 0 ? ans : new int[]{-1,-1};
		}

		private boolean isPrimeNumber(int num) {
			if (num < 2) {
				return false;
			}
			if (num == 2 || num == 3) {
				return true;
			}
			if (num % 2 == 0 || num % 3 == 0) {
				return false;
			}
			for (int i = 5; i * i <= num; i += 6) {
				if (num % i == 0 || num % (i + 2) == 0) {
					return false;
				}
			}
			return true;
		}
	}

	class SieveOfEratosthenesSolution { // O(N * log(log(N))), O(N)
		public int[] closestPrimes(int left, int right) {
			boolean[] isPrime = sieveOfEratosthenes(right);
			int[] ans = new int[2];
			int lastPrimeNum = 0;
			for (int i = left; i <= right; i++) {
				if (isPrime[i]) {
					if (ans[0] == 0) {
						ans[0] = i;
					} else if (ans[1] == 0) {
						ans[1] = i;
					} else {
						if (ans[1] - ans[0] > i - lastPrimeNum) {
							ans[0] = lastPrimeNum;
							ans[1] = i;
						}
					}
					lastPrimeNum = i; 
				}
			}
			return ans[0] > 0 && ans[1] > 0 ? ans : new int[]{-1,-1};
		}

		private boolean[] sieveOfEratosthenes(int n) {
			boolean[] isPrime = new boolean[n + 1];
			Arrays.fill(isPrime, true);
			isPrime[0] = isPrime[1] = false;
			for (int i = 2; i * i <= n; i++) {
				if (isPrime[i]) {
					for (int j = i * i; j <= n; j += i) {
						isPrime[j] = false;
					}
				}
			}
			return isPrime;
		}
	}
}
