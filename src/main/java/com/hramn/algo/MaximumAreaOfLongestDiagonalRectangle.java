 ...
 * Diagonal length = sqrt(8 * 8 + 6 * 6) = sqrt(100) = 10.
 * So, the rectangle at index 1 has a greater diagonal length therefore 
 * we return area = 8 * 6 = 48.
 * 
 * Example 2:
 * Input: dimensions = [[3,4],[4,3]]
 * Output: 12
 * Explanation: Length of diagonal is the same for both which is 5, 
 * so maximum area = 12.
 * 
 * Constraints:
...
public class MaximumAreaOfLongestDiagonalRectangle {
	class Solution { // O(N), O(1) where N = dimensions.length
		public int areaOfMaxDiagonal(int[][] dimensions) {
			int maxArea = 0;
			int maxDiagonal = 0;
			for (int[] rec : dimensions) {
				int diagonal = (rec[0] * rec[0]) + (rec[1] * rec[1]);
