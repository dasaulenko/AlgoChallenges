 ...
 * a different word or phrase using all the original letters exactly once. For 
...
public class FindResultantArrayAfterRemovingAnagrams {
	class Solution { // O(N*M), O(1), where N = words.length, M = max length of words[i]
		public List<String> removeAnagrams(String[] words) {
			...
			for (int i = 1; i < n; i++) {
				int[] tmp = countLetters(words[i]);
				if (isAnagram(letters, tmp)) continue;
				result.add(word);
				word = words[i];
				letters = tmp;
			}
