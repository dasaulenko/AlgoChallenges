 ...
 * 
 * A spreadsheet is a grid with 26 columns (labeled from 'A' to 'Z') and 
 * a given number of rows. Each cell in the spreadsheet can hold an integer 
 ...
 *   where the letter represents the column (from 'A' to 'Z') and the number 
 ...
 * cell references or non-negative integers with values less than or equal 
 * to 10^5.
...
public class DesignSpreadsheet {
	class Spreadsheet {
		...
		private Map<String, Integer> cellToValue;
		
		public Spreadsheet(int rows) { // O(1), O(1)
			cellToValue = new HashMap<>();
		}

		public void setCell(String cell, int value) { // O(1), O(1)
			cellToValue.put(cell, value);
		}

		public void resetCell(String cell) { // O(1), O(1)
			cellToValue.remove(cell);
		}

		public int getValue(String formula) { // O(N), O(1) where N = formula.length()
			int index = formula.indexOf("+");
			String val1 = formula.substring(1, index);
			String val2 = formula.substring(index + 1);
			int num1 = getNum(val1);
			int num2 = getNum(val2);
			return num1 + num2;
		}
		...
		private int getNum(String val) {
			char f = val.charAt(0);
			if (f >= 'A' && f <= 'Z') {
				return cellToValue.getOrDefault(val, 0);
			}
